...
/** Finds all paths from a source node to a destination node using an iterative form of depth first search. This function can handle cyclic graphs by tracking visited nodes and aborts a traversal attempt if a node is visited more than once. **/
export function findAllPathsDfsIterative(graph: AdjacencyList, source:GraphNode, destination:GraphNode) {
    const queue = [[source]];
